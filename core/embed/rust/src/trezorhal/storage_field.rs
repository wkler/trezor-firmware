...
impl<T> Field<T> {
    ...
    // pub fn get(&self) -> StorageResult<usize> {
    //     if true {
    //         Ok(1 as usize)
    //     } else {
...
impl<const N: usize> Field<String<N>> {
    ...
    pub fn set(&self, val: String<N>) -> StorageResult<()> {
...
mod tests {
    ...
    fn test_value_too_long_str() {
        ...
        // Input too long
        // WARNING: this fails with a different reason, it panics
        // in String::from because the length is too long
        assert!(PUBLIC_STRING
            .set(String::from("someveryverylongstring"))
            .is_err());
        assert_eq!(PUBLIC_STRING.get(), Some(String::from("hello")));
    }
